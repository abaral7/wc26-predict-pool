...
export function makeQFMatches() {
  return fixtures.matches
    .filter((m) => m.stage === "knockout" && m.group === "QF")
    .map((m) => ({ ...m, winners: [...m.winners] }));
}

export function makeSFMatches() {
  return fixtures.matches
    .filter((m) => m.stage === "knockout" && m.group === "SF")
    .map((m) => ({ ...m, winners: [...m.winners] }));
}

export function make3PMatch() {
  return fixtures.matches
    .filter((m) => m.stage === "knockout" && m.group === "3P")
    .map((m) => ({ ...m, winners: [...m.winners] }));
}

export function makeFinalMatch() {
  return fixtures.matches
    .filter((m) => m.stage === "knockout" && m.group === "F")
    .map((m) => ({ ...m, winners: [...m.winners] }));
}
